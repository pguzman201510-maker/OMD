...
class BondLogic:
    ...
    @staticmethod
    def calculate_values(row_data, uvr_spot):
        """
        Calculates Nominal COP and Valor Costo based on bond data.
        Params:
            row_data (dict): Dictionary with keys 'Nominal', 'Price', 'Denom'
            uvr_spot (float): Current UVR value
        Returns:
            dict: { 'Nominal_COP': float, 'Valor_Costo': float }
        """
        nominal_orig = float(row_data.get("Nominal", 0.0))
        denom = row_data.get("Denom", "COP")
        price_pct = float(row_data.get("Price", 0.0)) / 100.0

        # Nominal COP
        nominal_cop = nominal_orig
        if denom == "UVR":
            nominal_cop = nominal_orig * uvr_spot

        # Valor Costo (Dirty Price * Nominal COP)
        # For UVR: Nominal_UVR * UVR * Price% = Nominal_COP * Price%
        # For COP: Nominal_COP * Price%
        valor_costo = nominal_cop * price_pct
